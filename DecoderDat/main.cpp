/*
	...
	Updated and distributed by Robertas64
...
int main()//
{
	cout << "------------------------------------\nItemsData Decoder\nUpdated by Robertas64\nGithub : github.com/robertas64\nOriginal Sources : GrowtopiaNoobs\n------------------------------------" << endl;
	json jdata;
	std::ifstream file("items.dat", std::ios::binary | std::ios::ate);
	int size = file.tellg();
	if (size == -1) {
		cout << "[!] Failed, make sure that items.dat is in the current directory" << std::endl;
		return 0;
	}
	...
	if (file.read((char*)(data), size))
	{
		...
		cout << "[!] Decoding items.dat failed." << endl;
#ifndef __linux__
		_getch();
#endif
		exit(0);
	...
	for (int i = 0; i < itemCount; i++) {
		...
		if (i != itemID)
			cout << "[+] ERROR : Item are unordered!" << endl;
		j["itemID"] = itemID;
	...
	cout << "[+] Items.dat has been decoded" << endl;
#ifndef __linux__
	_getch();
#endif
	return 0;
